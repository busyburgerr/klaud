...
export async function logAction(actorId, action, targetType, targetId, reason = null, details = null) {
  await run(
...
const serialize = async (row) =>
  S.listing(row, { images: (await imagesFor([row.id])).get(row.id) || [] });

// GET /api/moderation/stats — счётчики для панели
moderationRouter.get("/stats", async (_req, res) => {
  res.json({
    pending: (await get("SELECT COUNT(*) AS c FROM listings WHERE status = 'pending'")).c,
    rejected: (await get("SELECT COUNT(*) AS c FROM listings WHERE status = 'rejected'")).c,
    active: (await get("SELECT COUNT(*) AS c FROM listings WHERE status = 'active'")).c,
...
moderationRouter.get("/queue", async (req, res) => {
  ...
  const total = (await get("SELECT COUNT(*) AS c FROM listings WHERE status = ?", status)).c;
...
moderationRouter.post(
  "/listings/:id/approve",
  wrap(async (req, res) => {
    const row = await listingRow(req.params.id);
      ...
      await run(
        `UPDATE listings
            SET status = 'active', reject_reason = NULL, moderated_by = ?,
                moderated_at = now_utc(), updated_at = now_utc()
          WHERE id = ?`,
        req.user.id, row.id,
      );
      await logAction(req.user.id, "listing.approve", "listing", row.id);
    });

    res.json({ listing: await serialize(await listingRow(row.id)) });
  }),
);
...
moderationRouter.post(
  ...
  wrap(async (req, res) => {
    ...
    await tx(async () => {
      ...
      await logAction(req.user.id, "listing.reject", "listing", row.id, body.reason);
    });
...
moderationRouter.post(
  ...
  wrap(async (req, res) => {
    ...
    await tx(async () => {
      await run(
        `UPDATE listings
            SET status = 'archived', moderated_by = ?, moderated_at = now_utc(),
                updated_at = now_utc()
          WHERE id = ?`,
        req.user.id, row.id,
      );
      await logAction(req.user.id, "listing.archive", "listing", row.id, body.reason || null);
    });
...
moderationRouter.post(
  ...
  wrap(async (req, res) => {
    ...
    await tx(async () => {
      await run(
        `UPDATE reports SET status = ?, resolved_by = ?, resolved_at = now_utc() WHERE id = ?`,
...
moderationRouter.get("/log", async (req, res) => {
  const { limit, page, offset } = pagination(req.query, { defaultLimit: 50 });

  const total = (await get("SELECT COUNT(*) AS c FROM moderation_log")).c;
  const rows = await all(
