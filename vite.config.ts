...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    server: {
      ...
      strictPort: true,
      watch: { ignored: ['**/.figma/**', '**/data/**', '**/server/uploads/**'] },
      // Бэкенд Express слушает отдельный порт; фронтенд ходит по тем же
      // относительным путям (/api, /uploads), что и в проде.
      proxy: {
        '/api': { target: `http://localhost:${process.env.API_PORT || 3001}`, changeOrigin: true },
        '/uploads': { target: `http://localhost:${process.env.API_PORT || 3001}`, changeOrigin: true },
      },
    },
    preview: {
      host: '0.0.0.0',
